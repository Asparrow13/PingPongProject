 ...
 * @version 1
 */
public class SpaceBall extends SmoothMover
{
    ...
    {
        if(isTouching(SpacePaddle.class))
        {
            if (! hasBouncedVertically)
            {
                ...
                if (hitCount % HIT_THRESHOLD == 0)
                {
                    ...
                    ((SpacePingWorld) getWorld()).increaseGameLevel();// Increase speed slightly
                }
            }
            else
            {
    ...
    {
    SpaceSelfMovingPaddle selfPaddle = (SpaceSelfMovingPaddle) getOneIntersectingObject(SpaceSelfMovingPaddle.class);
    ...
    private void checkRestart()
    {
        if (isTouchingFloor())
        {
            ...
            ((SpacePingWorld) getWorld()).resetGameLevel();
